...
export function computeTaxLiability(profile: TaxProfile): TaxResult {
  ...
  const qbiDeductionCandidate = Math.min(qbi * 0.2, Math.max(0, agi - standardDeduction) * 0.2)
  ...
  // ── Step 5: Federal Tax ──────────────────────────────────────────────────────
  const { tax: federalTax, marginalRate: marginalFederalRate } = applyBrackets(taxableIncome, brackets)
